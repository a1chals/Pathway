...
import { X, TrendingUp, TrendingDown, Clock, GraduationCap, Building2 } from "lucide-react";
...
export default function CompanyDetailPanel({ node, onClose }: CompanyDetailPanelProps) {
  if (!node) return null;

  const mostCommonExitType = node.topExitCompanies.length > 0 
    ? "Tech & Finance"
    : "Various";
      ...
      >
        ...
        <div className="sticky top-0 bg-gray-900/95 backdrop-blur-xl border-b border-gray-700 p-6 z-10">
          ...
          <div className="flex items-start gap-4">
            ...
            >
              {node.name.split(/[\s&]+/).slice(0, 2).map(w => w[0]).join("").toUpperCase()}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div
                ...
                style={{
                  backgroundColor: typeColors[node.type] + "30",
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-white">
                  {node.avgYearsBeforeExit.toFixed(1)} years
                </div>
              </div>
              ...
              {/* MBA Percentage */}
              {node.mbaPercentage !== undefined && node.mbaPercentage > 0 && (
                <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-1">
          ...
          {/* Top Exit Companies */}
          {node.topExitCompanies.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider mb-4">
                Top Exit Companies
              </h3>
              ...
              <div className="space-y-3">
                {node.topExitCompanies.map((exit, index) => (
                  <motion.div
                    key={exit.company}
                  ...
                  >
                    ...
                    <div className="w-full bg-gray-700/50 rounded-full h-1.5 mt-2">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${(exit.count / node.outgoing) * 100}%` }}
