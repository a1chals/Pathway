...
/**
 * 3D Adaptive Vertical Navigation Pill - For sidebar on other pages
 */
export const PathwayNavigationVertical: React.FC = () => {
  ...
  const navItems: NavItem[] = [
    ...
    { label: 'Movement', id: 'movement', path: '/heatmap' },
    { label: 'Pipeline', id: 'pipeline', path: '/incoming' },
    { label: 'Compare', id: 'compare', path: '/compare' },
  ]

  // Spring animations for smooth motion
  const pillHeight = useSpring(56, { stiffness: 220, damping: 25, mass: 1 })
  ...
  useEffect(() => {
    ...
    if (currentItem) {
      ...
      prevSectionRef.current = currentItem.id
    }
  }, [pathname])
  ...
  return (
    ...
    >
      {/* Primary edge ridge - ultra bright */}
      <div 
        className="absolute inset-y-0 left-0 rounded-l-full pointer-events-none"
        style={{
          width: '2px',
          background: 'linear-gradient(0deg, rgba(255, 255, 255, 0) 0%, rgba(255, 255, 255, 0.95) 5%, rgba(255, 255, 255, 1) 15%, rgba(255, 255, 255, 1) 85%, rgba(255, 255, 255, 0.95) 95%, rgba(255, 255, 255, 0) 100%)',
          filter: 'blur(0.3px)',
        }}
      />
      
      {/* Left hemisphere light catch */}
      <div 
        className="absolute inset-y-0 left-0 rounded-full pointer-events-none"
        style={{
          width: '55%',
          background: 'linear-gradient(90deg, rgba(255, 255, 255, 0.45) 0%, rgba(255, 255, 255, 0.25) 30%, rgba(255, 255, 255, 0.10) 60%, rgba(255, 255, 255, 0) 100%)',
        }}
      />
      ...
      {/* Top edge illumination - only show when expanded */}
      {expanded && (
        <div 
          className="absolute inset-x-0 top-0 rounded-t-full pointer-events-none"
          style={{
            height: '35%',
            background: 'linear-gradient(180deg, rgba(255, 255, 255, 0.20) 0%, rgba(255, 255, 255, 0.10) 40%, rgba(255, 255, 255, 0.03) 70%, rgba(255, 255, 255, 0) 100%)',
          }}
        />
      )}
      
      {/* Bottom edge shadow - only show when expanded */}
      {expanded && (
        <div 
          className="absolute inset-x-0 bottom-0 rounded-b-full pointer-events-none"
          style={{
            height: '35%',
            background: 'linear-gradient(0deg, rgba(0, 0, 0, 0.10) 0%, rgba(0, 0, 0, 0.05) 40%, rgba(0, 0, 0, 0.02) 70%, rgba(0, 0, 0, 0) 100%)',
          }}
        />
      )}
      
      {/* Right curvature - deep shadow */}
      <div 
        className="absolute inset-y-0 right-0 rounded-r-full pointer-events-none"
        style={{
          width: '50%',
          background: 'linear-gradient(270deg, rgba(0, 0, 0, 0.14) 0%, rgba(0, 0, 0, 0.08) 25%, rgba(0, 0, 0, 0.03) 50%, rgba(0, 0, 0, 0) 100%)',
        }}
      />

      {/* Right edge contact shadow */}
      <div 
        className="absolute inset-y-0 right-0 rounded-r-full pointer-events-none"
        style={{
          width: '20%',
      ...
      <div 
        ref={containerRef}
        className="relative z-10 h-full w-full flex flex-col items-center justify-center py-6 px-2"
        style={{
          fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "SF Pro", var(--font-mono), sans-serif',
        }}
      >
        {/* Collapsed state - show only active section initial */}
        {!expanded && (
              ...
              {activeItem && (
                <motion.span
                  ...
                  style={{
                    ...
                    whiteSpace: 'nowrap',
                    fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "SF Pro Display", var(--font-mono), sans-serif',
        ...
        {expanded && (
          <div className="flex flex-col items-center justify-evenly h-full w-full gap-2">
