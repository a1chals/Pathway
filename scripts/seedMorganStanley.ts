/**
 * Seed Script - Morgan Stanley Only
 */
...
// USA Detection
const US_INDICATORS = ['united states', 'usa', 'u.s.', 'california', 'new york', 'texas', 'illinois', 'massachusetts', 'washington', 'florida', 'georgia', 'pennsylvania', 'ohio', 'michigan', 'new jersey', 'connecticut', 'colorado', 'arizona', 'san francisco', 'los angeles', 'chicago', 'boston', 'nyc', 'manhattan', 'seattle', 'denver', 'atlanta', 'miami', 'dallas', 'houston', 'austin', 'phoenix', 'portland', 'minneapolis', 'detroit', 'philadelphia', 'dc', 'd.c.', 'stamford', 'greenwich'];

function isUSA(location: string | null): boolean {
  if (!location) return false;
  const loc = location.toLowerCase();
  return US_INDICATORS.some(ind => loc.includes(ind));
}
...
function categorizeCompany(name: string): string {
  ...
  if (/bridgewater|citadel|two sigma|de shaw|point72|millennium|elliott|third point|baupost|tiger global|renaissance|jane street|hudson river|worldquant|chilton/i.test(n)) return 'Hedge Fund';
  if (/goldman sachs|morgan stanley|jpmorgan|jp morgan|bank of america|merrill lynch|citi|credit suisse|deutsche bank|barclays|ubs|evercore|lazard|centerview|moelis|pjt|perella|qatalyst|greenhill|rothschild|jefferies|hsbc|nomura|rbc|td securities|baird|houlihan|william blair|raymond james|guggenheim|bnp paribas|bny|texas capital|dtcc/i.test(n)) return 'Investment Banking';
  ...
  if (/google|alphabet|meta|facebook|amazon|microsoft|apple|netflix|uber|airbnb|stripe|salesforce|snowflake|databricks|palantir|openai|slack|zoom|shopify|robinhood|coinbase|doordash|lyft|twitter|linkedin|oracle|adobe|ibm|nvidia|tesla|tiktok|bloomberg|mastercard|visa|verizon|indeed|nextdoor|cognizant/i.test(n)) return 'Big Tech';
  if (/blackrock|fidelity|vanguard|state street|pimco|wellington|t. rowe|franklin|invesco|asset management|wealth|advisors|calstrs|epoch|cerity|mariner/i.test(n)) return 'Asset Management';
  if (/aflac|insurance|prudential|aetna|cigna|humana|united health|kaiser|anthem|delta dental|express scripts/i.test(n)) return 'Corporate';
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Morgan Stanley');
  console.log('════════════════════════════════════════════════════════════\n');

  // Search company
  console.log('🔍 Searching for Morgan Stanley...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'Morgan Stanley', limit: 1, offset: 0 } }),
  });
  totalApiCalls++;
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    const sourceExp = sortedExp.find((exp: any) => {
      const n = (exp.companyName || '').toLowerCase();
      return MORGAN_STANLEY.aliases.some(a => n.includes(a));
    });
    ...
    for (const exp of sortedExp) {
      const n = (exp.companyName || '').toLowerCase();
      if (MORGAN_STANLEY.aliases.some(a => n.includes(a))) continue;
    ...
    const { error } = await supabase.from('exits').upsert({
      ...
      source_company_name: 'Morgan Stanley',
  ...
  // Final stats
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ MORGAN STANLEY COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
