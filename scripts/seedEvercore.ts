...
function isEvercore(name: string): boolean {
  const n = name.toLowerCase();
  return EVERCORE.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Evercore');
  console.log('════════════════════════════════════════════════════════════\n');

  console.log('🔍 Searching for Evercore...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'Evercore', limit: 5, offset: 0 } }),
  });
  totalApiCalls++;
  
  const searchData = await searchRes.json();
  const company = searchData.items?.find((c: any) => 
    c.name.toLowerCase().includes('evercore')
  );
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isEvercore(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isEvercore(exp.companyName || '')) continue;
