...
const LEK = {
  ...
  aliases: ['l.e.k. consulting', 'lek consulting', 'l.e.k.', 'lek']
};
...
function isLEK(name: string): boolean {
  const n = name.toLowerCase();
  return LEK.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: L.E.K. Consulting');
  console.log('════════════════════════════════════════════════════════════\n');

  console.log('🔍 Searching for L.E.K. Consulting...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'L E K', limit: 10, offset: 0 } }),
  });
  totalApiCalls++;
  
  const searchData = await searchRes.json();
  const company = searchData.items?.find((c: any) => 
    c.name.toLowerCase().includes('l.e.k') || c.name.toLowerCase().includes('lek')
  );
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isLEK(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isLEK(exp.companyName || '')) continue;
      ...
      const { error } = await supabase.from('exits').upsert({
        ...
        source_company_name: 'L.E.K. Consulting',
  ...
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ L.E.K. CONSULTING COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
