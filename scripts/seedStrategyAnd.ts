/**
 * Seed Script - Strategy& (PwC's strategy consulting arm, formerly Booz & Company)
 */
...
// Strategy& ID found from search
const STRATEGY_ID = 'Igw6eIEgea0roHxShX1RH38JrYo3VpH';

const STRATEGY = {
  displayName: 'Strategy&',
  aliases: ['strategy&', 'strategy and', 'booz & company', 'booz and company', 'booz']
};
...
function isStrategy(name: string): boolean {
  const n = name.toLowerCase();
  return STRATEGY.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Strategy& (PwC Strategy Consulting)');
  console.log('════════════════════════════════════════════════════════════\n');

  const companyId = STRATEGY_ID;
  console.log(`✅ Using Strategy& (ID: ${companyId})`);
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isStrategy(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isStrategy(exp.companyName || '')) continue;
