/**
 * Seed Script - Boston Consulting Group (BCG) Only
 */
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Boston Consulting Group (BCG)');
  console.log('════════════════════════════════════════════════════════════\n');

  // Search company
  console.log('🔍 Searching for BCG...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'Boston Consulting', limit: 1, offset: 0 } }),
  });
  totalApiCalls++;
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    const sourceExp = sortedExp.find((exp: any) => {
      ...
      return BCG.aliases.some(a => n.includes(a));
    });
    ...
    for (const exp of sortedExp) {
      const n = (exp.companyName || '').toLowerCase();
      if (BCG.aliases.some(a => n.includes(a))) continue;
    ...
    const { error } = await supabase.from('exits').upsert({
      ...
      source_company_name: 'Boston Consulting Group',
  ...
  // Final stats
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ BOSTON CONSULTING GROUP (BCG) COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
