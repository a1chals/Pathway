/**
 * Seed Script - Lazard
 * Aliases: Lazard Freres, Lazard Asset Management
 */
...
function isLazard(name: string): boolean {
  const n = name.toLowerCase();
  return LAZARD.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Lazard');
  console.log('════════════════════════════════════════════════════════════\n');

  console.log('🔍 Searching for Lazard...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'Lazard', limit: 10, offset: 0 } }),
  });
  totalApiCalls++;
  ...
  // Find main Lazard entity (not regional)
  const company = searchData.items?.find((c: any) => {
    const n = c.name.toLowerCase();
    return n === 'lazard' || n === 'lazard ltd' || (n.includes('lazard') && !n.includes('uk') && !n.includes('canada') && !n.includes('australia'));
  }) || searchData.items?.find((c: any) => c.name.toLowerCase().includes('lazard'));
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isLazard(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isLazard(exp.companyName || '')) continue;
