/**
 * Seed Script - Bain & Company Only
 * Note: This is for Bain & Company (consulting), NOT Bain Capital (PE)
 */
...
const BAIN = {
  ...
  // Important: exclude "bain capital" - that's PE, not consulting
  aliases: ['bain & company', 'bain and company', 'bain & co']
};
...
// Check if company name is Bain & Company (not Bain Capital)
function isBainCompany(name: string): boolean {
  const n = name.toLowerCase();
  // Must contain "bain" but NOT "capital"
  if (n.includes('bain capital')) return false;
  return BAIN.aliases.some(a => n.includes(a)) || (n.includes('bain') && !n.includes('capital'));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Bain & Company');
  console.log('════════════════════════════════════════════════════════════\n');

  // Search company
  console.log('🔍 Searching for Bain & Company...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'Bain & Company', limit: 5, offset: 0 } }),
  });
  totalApiCalls++;
  
  const searchData = await searchRes.json();
  // Find the actual Bain & Company (not Bain Capital)
  const company = searchData.items?.find((c: any) => 
    c.name.toLowerCase().includes('bain') && 
    !c.name.toLowerCase().includes('capital')
  );
  
  if (!company) {
    console.log('❌ Company not found');
    console.log('Search results:', searchData.items?.map((c: any) => c.name));
    return;
  }
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      // Find source experience (Bain & Company)
      ...
      const sourceExp = sortedExp.find((exp: any) => isBainCompany(exp.companyName || ''));
      ...
      // Find next job (exclude Bain & Company but allow Bain Capital as exit)
      let nextJob = null;
      for (const exp of sortedExp) {
        if (isBainCompany(exp.companyName || '')) continue;
      ...
      const { error } = await supabase.from('exits').upsert({
        ...
        source_company_name: 'Bain & Company',
  ...
  // Final stats
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ BAIN & COMPANY COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
