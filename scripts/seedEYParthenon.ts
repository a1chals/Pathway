/**
 * Seed Script - EY-Parthenon (Strategy Consulting)
 * More focused than main EY entity
 */
...
const EY_PARTHENON = {
  ...
  aliases: ['ey-parthenon', 'ey parthenon', 'parthenon-ey', 'parthenon group', 'parthenon ey']
};
...
function isEYParthenon(name: string): boolean {
  const n = name.toLowerCase();
  return EY_PARTHENON.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: EY-Parthenon');
  console.log('════════════════════════════════════════════════════════════\n');

  // Try multiple search terms
  const searchTerms = ['EY-Parthenon', 'Parthenon Group', 'Parthenon-EY'];
  ...
  for (const term of searchTerms) {
    ...
    const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify({ dsl: { nameQuery: term, limit: 10, offset: 0 } }),
    });
    totalApiCalls++;
    
    const searchData = await searchRes.json();
    console.log('Results:', searchData.items?.map((c: any) => `${c.name} (${c.id})`));
    
    const match = searchData.items?.find((c: any) => {
      const n = c.name.toLowerCase();
      return n.includes('parthenon');
    });
  ...
  if (!company) {
    console.log('❌ EY-Parthenon not found, trying broader EY search...');
    // Fall back to main EY
    await sleep(RATE_LIMIT_MS);
    const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify({ dsl: { nameQuery: 'EY', limit: 10, offset: 0 } }),
    });
    totalApiCalls++;
    const searchData = await searchRes.json();
    company = searchData.items?.find((c: any) => c.name === 'EY');
    if (!company) {
      console.log('❌ No EY entity found');
      return;
    }
  }
  
  console.log(`\n✅ Found: ${company.name} (ID: ${company.id})`);
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      // Look for EY-Parthenon or Parthenon experience
      const sourceExp = sortedExp.find((exp: any) => {
        const n = (exp.companyName || '').toLowerCase();
        return n.includes('parthenon') || n.includes('ey-parthenon');
      });
      ...
      for (const exp of sortedExp) {
        const n = (exp.companyName || '').toLowerCase();
        if (n.includes('parthenon') || n.includes('ey-parthenon')) continue;
      ...
      const { error } = await supabase.from('exits').upsert({
        ...
        source_company_name: 'EY-Parthenon',
  ...
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ EY-PARTHENON COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
