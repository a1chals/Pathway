/**
 * Seed Script - Kearney (formerly A.T. Kearney)
 */
...
function isKearney(name: string): boolean {
  const n = name.toLowerCase();
  return KEARNEY.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Kearney (A.T. Kearney)');
  console.log('════════════════════════════════════════════════════════════\n');

  // Try both names
  const searchTerms = ['Kearney', 'A.T. Kearney'];
  let company = null;
  ...
  for (const term of searchTerms) {
    ...
    const match = searchData.items?.find((c: any) => {
      ...
      return n.includes('kearney') && !n.includes('real estate');
    });
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isKearney(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isKearney(exp.companyName || '')) continue;
