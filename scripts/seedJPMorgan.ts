/**
 * Seed Script - J.P. Morgan (Investment Banking)
 * Using the specific J.P. Morgan entity ID
 */
...
// Use J.P. Morgan specifically (investment banking)
const JPMORGAN_ID = '02nTEbvdJ7RaCvba68kr67Ez7KuYAIU';

const JPMORGAN = {
  displayName: 'J.P. Morgan',
  aliases: ['jpmorgan', 'j.p. morgan', 'jp morgan', 'jpmorgan chase']
};
...
function isJPMorgan(name: string): boolean {
  const n = name.toLowerCase();
  return JPMORGAN.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: J.P. Morgan (Investment Banking)');
  console.log('════════════════════════════════════════════════════════════\n');

  // Use the specific J.P. Morgan ID directly
  const companyId = JPMORGAN_ID;
  console.log(`✅ Using J.P. Morgan (ID: ${companyId})`);
  ...
  while (page <= totalPages && allEmployees.length < MAX_EMPLOYEES) {
    ...
    try {
      ...
      if (!empRes.ok) {
        ...
        console.log(`  ❌ Page ${page} failed: ${empRes.status} - ${text.slice(0, 100)}`);
        consecutiveErrors++;
        if (consecutiveErrors >= 3) {
          console.log('  ⚠️ Too many consecutive errors, stopping fetch');
          break;
        }
        page++;
        continue;
  ...
  if (allEmployees.length === 0) {
    console.log('❌ No employees fetched. API may be having issues.');
    return;
  }
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isJPMorgan(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isJPMorgan(exp.companyName || '')) continue;
      ...
      const { error } = await supabase.from('exits').upsert({
        ...
        source_company_name: 'J.P. Morgan',
  ...
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ J.P. MORGAN COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
