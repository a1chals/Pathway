/**
 * Seed Script - Centerview Partners
 */
...
const CENTERVIEW = {
  ...
  aliases: ['centerview', 'centerview partners']
};
...
function isCenterview(name: string): boolean {
  const n = name.toLowerCase();
  return CENTERVIEW.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Centerview Partners');
  console.log('════════════════════════════════════════════════════════════\n');

  console.log('🔍 Searching for Centerview Partners...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'Centerview Partners', limit: 10, offset: 0 } }),
  });
  totalApiCalls++;
  ...
  const company = searchData.items?.find((c: any) => 
    c.name.toLowerCase().includes('centerview')
  );
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isCenterview(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isCenterview(exp.companyName || '')) continue;
      ...
      const { error } = await supabase.from('exits').upsert({
        ...
        source_company_name: 'Centerview Partners',
  ...
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ CENTERVIEW PARTNERS COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
