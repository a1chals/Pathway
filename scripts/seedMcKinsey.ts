/**
 * Seed Script - McKinsey & Company Only
 */
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: McKinsey & Company');
  console.log('════════════════════════════════════════════════════════════\n');

  // Search company
  console.log('🔍 Searching for McKinsey...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'McKinsey & Company', limit: 1, offset: 0 } }),
  });
  totalApiCalls++;
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    // Find source experience (McKinsey)
    ...
    const sourceExp = sortedExp.find((exp: any) => {
      ...
      return MCKINSEY.aliases.some(a => n.includes(a));
    });
    ...
    for (const exp of sortedExp) {
      const n = (exp.companyName || '').toLowerCase();
      if (MCKINSEY.aliases.some(a => n.includes(a))) continue;
    ...
    const { error } = await supabase.from('exits').upsert({
      ...
      source_company_name: 'McKinsey & Company',
  ...
  // Final stats
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ MCKINSEY & COMPANY COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
