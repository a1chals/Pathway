/**
 * Seed Script - KPMG
 * Aliases: KPMG Advisory, KPMG Strategy
 */
...
const KPMG = {
  searchName: 'KPMG',
  displayName: 'KPMG',
  aliases: ['kpmg', 'kpmg advisory', 'kpmg strategy', 'kpmg consulting', 'kpmg llp', 'kpmg us']
};
...
function isKPMG(name: string): boolean {
  const n = name.toLowerCase();
  return KPMG.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: KPMG');
  console.log('════════════════════════════════════════════════════════════\n');

  console.log('🔍 Searching for KPMG...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'KPMG', limit: 10, offset: 0 } }),
  });
  totalApiCalls++;
  ...
  // Find the main KPMG US entity
  const company = searchData.items?.find((c: any) => {
    const n = c.name.toLowerCase();
    return n === 'kpmg' || n === 'kpmg llp' || n === 'kpmg us' || (n.includes('kpmg') && c.country === 'United States');
  }) || searchData.items?.find((c: any) => c.name.toLowerCase().includes('kpmg'));
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isKPMG(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isKPMG(exp.companyName || '')) continue;
