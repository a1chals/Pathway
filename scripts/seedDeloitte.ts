/**
 * Seed Script - Monitor Deloitte (Strategy Consulting)
 */
...
// Use Monitor Deloitte
const DELOITTE_ID = 'JO3BD4JxyF-eFpyJ2x_cSpaGZ4QmMsh';

const DELOITTE = {
  displayName: 'Monitor Deloitte',
  aliases: ['deloitte', 'monitor deloitte', 'deloitte consulting', 'deloitte advisory', 'monitor group']
};
...
function isDeloitte(name: string): boolean {
  const n = name.toLowerCase();
  return DELOITTE.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: Monitor Deloitte');
  console.log('════════════════════════════════════════════════════════════\n');

  const companyId = DELOITTE_ID;
  console.log(`✅ Using Monitor Deloitte (ID: ${companyId})`);
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isDeloitte(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isDeloitte(exp.companyName || '')) continue;
      ...
      const { error } = await supabase.from('exits').upsert({
        ...
        source_company_name: 'Monitor Deloitte',
  ...
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ MONITOR DELOITTE COMPLETE');
  console.log('════════════════════════════════════════════════════════════');
