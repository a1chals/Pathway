/**
 * Seed Script - EY-Parthenon (3000 cap)
 */
...
const COMPANY = {
  ...
  aliases: ['ey-parthenon', 'parthenon', 'parthenon group', 'parthenon-ey']
};
...
function isSourceCompany(name: string): boolean {
  ...
  return COMPANY.aliases.some(a => n.includes(a));
}
...
async function fetchWithRetry(url: string, options: RequestInit, retries = 5): Promise<Response> {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (i < retries - 1) { await sleep(Math.pow(2, i + 1) * 1000); continue; }
      throw e;
    }
  }
...
function getHeaders() {
  return { 'Authorization': `Bearer ${AVIATO_API_KEY}`, 'Content-Type': 'application/json' };
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: EY-Parthenon (3000 cap)');
  console.log('════════════════════════════════════════════════════════════\n');

  // Get current count
  const { count: currentCount } = await supabase.from('exits').select('*', { count: 'exact', head: true }).eq('source_company_name', 'EY-Parthenon');
  console.log(`📈 Current exits: ${currentCount}`);
  if ((currentCount || 0) >= 150) {
    console.log('✅ Already has 150+ exits!');
    return;
  }

  console.log('🔍 Searching for EY-Parthenon...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'EY-Parthenon', limit: 10, offset: 0 } }),
  });
  totalApiCalls++;
  
  const searchData = await searchRes.json();
  const company = searchData.items?.find((c: any) => c.name.toLowerCase().includes('parthenon'));
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    if (i > 0 && i % 100 === 0) {
      const { count } = await supabase.from('exits').select('*', { count: 'exact', head: true }).eq('source_company_name', 'EY-Parthenon');
      console.log(`  Progress: ${i}/${allEmployees.length} | DB Exits: ${count} | Non-USA: ${skippedUSA} | Pre-2020: ${skipped2020}`);
      if ((count || 0) >= 150) {
        console.log('  ✅ Reached 150 exits!');
        break;
      }
    }
    ...
    try {
      ...
      for (const exp of sortedExp) {
        ...
        if (expStart && expStart >= sourceEnd) {
          if (!nextJob || expStart < new Date(nextJob.startDate)) nextJob = exp;
        }
      }
  ...
  const { count: finalCount } = await supabase.from('exits').select('*', { count: 'exact', head: true }).eq('source_company_name', 'EY-Parthenon');
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('✅ EY-PARTHENON COMPLETE');
  console.log(`  Final exits: ${finalCount}`);
