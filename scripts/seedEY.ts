/**
 * Seed Script - EY
 * Aliases: EY-Parthenon, Ernst & Young, Parthenon Group
 */
...
const EY = {
  searchName: 'EY',
  displayName: 'EY',
  aliases: ['ey', 'ey-parthenon', 'ernst & young', 'ernst and young', 'parthenon group', 'parthenon-ey', 'ey parthenon']
};
...
function isEY(name: string): boolean {
  const n = name.toLowerCase();
  return EY.aliases.some(a => n.includes(a));
}
...
async function main() {
  console.log('════════════════════════════════════════════════════════════');
  console.log('📊 Processing: EY (Ernst & Young / EY-Parthenon)');
  console.log('════════════════════════════════════════════════════════════\n');

  console.log('🔍 Searching for EY...');
  await sleep(RATE_LIMIT_MS);
  const searchRes = await fetchWithRetry(`${AVIATO_BASE_URL}/company/search`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ dsl: { nameQuery: 'Ernst & Young', limit: 10, offset: 0 } }),
  });
  totalApiCalls++;
  ...
  // Find the main EY entity (prefer US or global)
  const company = searchData.items?.find((c: any) => {
    const n = c.name.toLowerCase();
    return n === 'ey' || n === 'ernst & young' || (n.includes('ernst') && n.includes('young') && !n.includes('uk') && !n.includes('canada'));
  }) || searchData.items?.find((c: any) => c.name.toLowerCase().includes('ernst'));
  ...
  for (let i = 0; i < allEmployees.length; i++) {
    ...
    try {
      ...
      const sourceExp = sortedExp.find((exp: any) => isEY(exp.companyName || ''));
      ...
      for (const exp of sortedExp) {
        if (isEY(exp.companyName || '')) continue;
